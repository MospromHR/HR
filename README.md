...
Готовое демо-решение для управления воронкой найма: веб-интерфейс для HR-специалистов и API на FastAPI, объединённые в один стек с PostgreSQL и Nginx.
...
- [Настройка backend](#настройка-backend)
  ...
  - [Использование внешней базы данных](#использование-внешней-базы-данных)
  - [Хранилище медиа-файлов](#хранилище-медиа-файлов)
...
## Настройка backend
Backend построен на FastAPI и читает конфигурацию из трёх источников (в порядке приоритета):
...
| `superuser.role`    | Роль суперпользователя (`ADMIN` по умолчанию). |
| `storage.media_root` | Путь к директории хранения загруженных файлов. |
| `storage.public_path_prefix` | URL-префикс, по которому Nginx отдаёт медиа (`/backend-media`). |

Для использования кастомного файла достаточно смонтировать его при запуске контейнера backend:
```yaml
services:
  backend:
    volumes:
...
| `APP_SUPERUSER_ROLE` | `ADMIN` | Роль суперпользователя. |
...
APP_POSTGRES_HOST=db.example.com
APP_POSTGRES_PORT=5432
APP_POSTGRES_USERNAME=myuser
APP_POSTGRES_PASSWORD=mypass
APP_POSTGRES_DATABASE=hr
```

Затем поднимите стек без встроенного сервиса `postgres`:
```bash
...
Собранный бандл не привязан к конкретному домену: Nginx проксирует API по `/api`, поэтому можно развернуть фронтенд на любом хосте, изменив лишь доменное имя в настройках DNS/инфраструктуры.
